...
- 本自述文件对应脚本：`【MacOS】♻️双击修复：找不到 Android SDK 或 platforms 目录为空.command`。
- 脚本原始位置：`JobsGenesis@JobsCommand.Flutter/小工具`。
...
| 项目 | 说明 |
|---|---|
| 脚本名称 | `【MacOS】♻️双击修复：找不到 Android SDK 或 platforms 目录为空.command` |
| 所属目录 | `JobsGenesis@JobsCommand.Flutter/小工具` |
| 主要标签 | `通用脚本` |
| 是否涉及 Homebrew | `否` |
| 是否可能联网 | `是` |
| 是否含高风险命令 | `否` |
...
```shell
chmod +x './【MacOS】♻️双击修复：找不到 Android SDK 或 platforms 目录为空.command'
'./【MacOS】♻️双击修复：找不到 Android SDK 或 platforms 目录为空.command'
```
...
- 运行日志默认写入：`$TMPDIR/【MacOS】♻️双击修复：找不到 Android SDK 或 platforms 目录为空.log`。
