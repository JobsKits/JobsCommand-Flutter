# `【MacOS】☀️双击运行Flutter项目（Android 模拟器） .command`
...
- 本自述文件对应脚本：`【MacOS】☀️双击运行Flutter项目（Android 模拟器） .command`。
- 脚本原始位置：`JobsGenesis@JobsCommand.Flutter/运行`。
...
| 所属目录 | `JobsGenesis@JobsCommand.Flutter/运行` |
| 主要标签 | `iOS` |
| 是否涉及 Homebrew | `是` |
| 是否可能联网 | `是` |
| 是否含高风险命令 | `是` |
...
```shell
chmod +x './【MacOS】☀️双击运行Flutter项目（Android 模拟器） .command'
'./【MacOS】☀️双击运行Flutter项目（Android 模拟器） .command'
```
...
- 运行日志默认写入：`$TMPDIR/【MacOS】☀️双击运行Flutter项目（Android 模拟器） .log`。
