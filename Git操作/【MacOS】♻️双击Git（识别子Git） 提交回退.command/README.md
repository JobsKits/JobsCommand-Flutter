# `【MacOS】♻️双击Git（识别子Git） 提交回退.command`
...
- 本自述文件对应脚本：`【MacOS】♻️双击Git（识别子Git） 提交回退.command`。
- 脚本原始位置：`JobsGenesis@JobsCommand.Flutter/Git操作`。
...
| 项目 | 说明 |
|---|---|
| 脚本名称 | `【MacOS】♻️双击Git（识别子Git） 提交回退.command` |
| 所属目录 | `JobsGenesis@JobsCommand.Flutter/Git操作` |
| 主要标签 | `通用脚本` |
| 是否涉及 Homebrew | `是` |
| 是否可能联网 | `是` |
| 是否含高风险命令 | `是` |
...
```shell
chmod +x './【MacOS】♻️双击Git（识别子Git） 提交回退.command'
'./【MacOS】♻️双击Git（识别子Git） 提交回退.command'
```
...
- 运行日志默认写入：`$TMPDIR/【MacOS】♻️双击Git（识别子Git） 提交回退.log`。
