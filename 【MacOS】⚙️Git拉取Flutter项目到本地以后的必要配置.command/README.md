...
- 本自述文件对应脚本：`【MacOS】⚙️Git拉取Flutter项目到本地以后的必要配置.command`。
- 脚本原始位置：`JobsGenesis@JobsCommand.Flutter`。
...
| 所属目录 | `JobsGenesis@JobsCommand.Flutter` |
| 主要标签 | `通用脚本` |
| 是否涉及 Homebrew | `是` |
| 是否可能联网 | `是` |
| 是否含高风险命令 | `否` |
...
```shell
chmod +x './【MacOS】⚙️Git拉取Flutter项目到本地以后的必要配置.command'
'./【MacOS】⚙️Git拉取Flutter项目到本地以后的必要配置.command'
```
...
- 运行日志默认写入：`$TMPDIR/【MacOS】⚙️Git拉取Flutter项目到本地以后的必要配置.log`。
