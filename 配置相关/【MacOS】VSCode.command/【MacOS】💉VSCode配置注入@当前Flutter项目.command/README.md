# `【MacOS】💉VSCode配置注入@当前Flutter项目.command`
...
- 本自述文件对应脚本：`【MacOS】💉VSCode配置注入@当前Flutter项目.command`。
- 脚本原始位置：`JobsGenesis@JobsCommand.Flutter/配置相关/VSCode`。
- 脚本定位：用于打开或配置开发工具。
...
| 项目 | 说明 |
|---|---|
| 脚本名称 | `【MacOS】💉VSCode配置注入@当前Flutter项目.command` |
| 所属目录 | `JobsGenesis@JobsCommand.Flutter/配置相关/VSCode` |
| 主要标签 | `工具入口` |
| 是否涉及 Homebrew | `否` |
| 是否可能联网 | `是` |
| 是否含高风险命令 | `是` |
...
```shell
chmod +x './【MacOS】💉VSCode配置注入@当前Flutter项目.command'
'./【MacOS】💉VSCode配置注入@当前Flutter项目.command'
```
...
- 运行日志默认写入：`$TMPDIR/【MacOS】💉VSCode配置注入@当前Flutter项目.log`。
